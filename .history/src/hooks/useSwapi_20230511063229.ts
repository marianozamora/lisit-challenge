...
export async function getAllResidents (data: any) {
    const residentsData = await Promise.all(
        data.residents.map((resident: any) => {
            return generateRequestByUrl({ url: resident });
        }));

    return residentsData.map((resident: any) => resident.data);
}

export function useGetAllResidents(data: any) {
    return useQuery({
        queryKey: ['residents', residents],
        queryFn: () => getAllResidents(residents),
    })
}
